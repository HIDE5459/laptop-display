...
## 表示側のキーボード・マウスでホストを操作する

表示側の PC で打ってしまう問題を避けるため、その入力をホスト(Mac)へ
転送して操作できます(簡易 KVM)。**Mac → Mac でも同じように使えます**
(入力を再現するのはホスト側なので、表示側の OS は問いません)。

1. **Mac 側**:「Windows 側から Mac を操作する」の
   **「キーボードとマウスの操作を受け付ける」**をオンにする
   (**アクセシビリティの許可**が必要。オンにすると許可ダイアログが出ます)
2. **表示側**: 画面上で **F9**(またはツールバーの**「操作: オフ/オン」ボタン**)で転送モードをオン
   - **表示側が Mac の場合、F9 は macOS に取られることがあります**。
     ツールバーのボタンを使うか、システム設定 → キーボードで
     「F1、F2 などのキーを標準のファンクションキーとして使用」をオンにしてください
...
役割は既定で「**Mac = ホスト(配信)/ Windows = ディスプレイ(受信)**」ですが、
両方のアプリの**「この PC の役割」**から切り替えられます(切り替えると再起動します)。
これにより Mac 同士・Windows 同士・Windows をホストにする構成も使えます。

| 構成 | 画面のストリーミング | 拡張ディスプレイ | カーソル移動 / Option タップ | 入力転送 (KVM) |
|---|---|---|---|---|
| Mac → Windows | ○ | ○ | ○ | ○ |
| Mac → Mac | ○ | ○ | ○ | ○ |
| Windows → Windows | ○ | × | × | × |
...
**ホストが Windows の場合の制限**: 仮想ディスプレイ・カーソル移動・入力の再現は
いずれも macOS の API (CGVirtualDisplay / CGWarpMouseCursorPosition / CGEvent) を
使っているため動きません。**ミラーリングでの画面配信のみ**になり、
該当する設定カードは非表示になります。
Windows をホストにして拡張ディスプレイや KVM も使いたい場合は、
Windows 版のネイティブ実装(仮想ディスプレイドライバ、SetCursorPos、SendInput)が
別途必要です。
