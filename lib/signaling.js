...
function startSignaling({ port, publicDir, onPeerChange }) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type === 'hello' && (msg.role === 'sender' || msg.role === 'receiver')) {
        ...
        peers[role] = ws;
        if (role === 'receiver' && msg.screen && msg.screen.w > 0 && msg.screen.h > 0) {
          receiverScreen = { w: msg.screen.w, h: msg.screen.h };
        }
        notify();
